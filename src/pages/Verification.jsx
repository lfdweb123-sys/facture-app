...
export default function Verification() {
  ...
  const initialFormState = {
    ...
    birthDate: '', documentFront: null, documentBack: null,
    companyName: '', companyRegNumber: '', companyDoc: null
  };
    ...
    try {
      await updateDoc(doc(db, 'users', user.uid), {
        ...
        verificationData: {
          ...
          firstName: formData.firstName, lastName: formData.lastName, birthDate: formData.birthDate,
          companyName: formData.companyName || '', companyRegNumber: formData.companyRegNumber || '',
  ...
  if (user?.verificationStatus === 'approved') return <div className="max-w-lg mx-auto p-4 sm:p-6"><div className="bg-white rounded-2xl border p-8 text-center"><CheckCircle size={32} className="text-emerald-500 mx-auto mb-3"/><h2 className="text-lg font-bold">Compte vérifié ✅</h2><p className="text-sm text-gray-500">Toutes les fonctionnalités sont accessibles.</p></div></div>;
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-6 space-y-6">
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex gap-3">
        ...
        <div><p className="text-sm font-medium text-amber-900">Vérification requise</p><p className="text-xs text-amber-700">Pour débloquer toutes les fonctionnalités, faites vérifier votre identité.</p></div>
      </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {documentTypes.map(doc => (
                <button key={doc.value} type="button" onClick={()=>setFormData({...formData,documentType:doc.value})}
                  className={`flex items-center gap-2 p-3 rounded-xl border text-left text-sm ${formData.documentType===doc.value?'border-gray-900 bg-gray-50 text-gray-900 font-medium':'border-gray-200 text-gray-600'}`}>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <label className="flex flex-col items-center justify-center h-28 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-gray-300 bg-gray-50">
                {formData.documentFront ? <div className="text-center"><CheckCircle size={18} className="text-emerald-500"/><p className="text-xs text-emerald-600 truncate max-w-[150px]">{formData.documentFront.name}</p></div> : <div className="text-center"><Camera size={22} className="text-gray-400"/><p className="text-xs text-gray-500">Cliquez pour ajouter</p></div>}
            ...
            <div>
              ...
              <label className="flex flex-col items-center justify-center h-28 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-gray-300 bg-gray-50">
                {formData.documentBack ? <div className="text-center"><CheckCircle size={18} className="text-emerald-500"/><p className="text-xs text-emerald-600 truncate">{formData.documentBack.name}</p></div> : <div className="text-center"><Upload size={22} className="text-gray-400"/><p className="text-xs text-gray-500">Optionnel</p></div>}
          ...
          {isBusiness && (
            <div>
              ...
              <label className="flex flex-col items-center justify-center h-28 border-2 border-dashed border-blue-200 rounded-xl cursor-pointer hover:border-blue-300 bg-blue-50">
                {formData.companyDoc ? <div className="text-center"><CheckCircle size={18} className="text-emerald-500"/><p className="text-xs text-emerald-600 truncate">{formData.companyDoc.name}</p></div> : <div className="text-center"><FileText size={22} className="text-blue-400"/><p className="text-xs text-blue-600">Scanner RCCM</p></div>}
