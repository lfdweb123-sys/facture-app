...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      try {
        ...
        if (data) {
          // Ne charger que si les données existent VRAIMENT
  ...
  // Vérifier l'état des notifications navigateur
  useEffect(() => {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      toast.error(error.code === 'auth/requires-recent-login' ? 'Reconnectez-vous' : 'Erreur');
    }
    finally { setLoading(false); }
  };

  const handleVerifyEmail = async () => {
    try { await sendEmailVerification(auth.currentUser); toast.success('Email envoyé'); }
    catch { toast.error('Erreur'); }
  };
  ...
  const handleDeleteApiKey = async () => {
    ...
    try { await updateDoc(doc(db, 'users', user.uid), { apiKey: null, updatedAt: new Date().toISOString() }); setApiKey(''); toast.success('Clé révoquée'); }
    catch { toast.error('Erreur'); }
  };
  ...
  const handleAddWebhook = async () => {
    ...
    try {
      ...
      setWebhooks(updated); setWebhookForm({ url: '', events: ['payment.received'] }); setShowWebhookForm(false);
          ...
          {activeTab === 'profile' && (
            <div className="space-y-5">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div><label className={labelClass}>Téléphone</label><input type="tel" value={profileSettings.phone} onChange={e => setProfileSettings(prev => ({...prev, phone: e.target.value}))} className={inputClass} /></div>
          ...
          {activeTab === 'invoices' && (
            <div className="space-y-5">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div><label className={labelClass}>Prochain N°</label><input type="number" value={invoiceSettings.prochainNumero} onChange={e => setInvoiceSettings(prev => ({...prev, prochainNumero: parseInt(e.target.value)||1}))} className={inputClass} /></div>
              </div>
              ...
              {!restrictions.canUseApi ? (
                ...
                <>
                    ...
                    {apiKey ? (
                      <div className="space-y-3">
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={handleCopyApiKey} className="p-2.5 border border-gray-200 rounded-xl hover:bg-gray-50"><Copy size={14} /></button>
                          <button onClick={handleDeleteApiKey} className="p-2.5 border border-red-200 rounded-xl hover:bg-red-50 text-red-500"><Trash2 size={14} /></button>
                        </div>
                      </div>
                    ) : (
                  ...
                  <div className="pt-6 border-t border-gray-100">
                    <div className="flex items-center justify-between mb-4"><div><h3 className="text-sm font-semibold">Webhooks</h3></div><button onClick={() => setShowWebhookForm(!showWebhookForm)} className="text-sm font-medium hover:underline flex items-center gap-1"><Plus size={14} /> Ajouter</button></div>
                    {showWebhookForm && (
                      <div className="bg-gray-50 rounded-xl p-4 mb-4 space-y-3">
                        ...
                        <div className="flex gap-2"><button onClick={handleAddWebhook} className="bg-gray-900 text-white px-4 py-2 rounded-lg text-xs">Ajouter</button><button onClick={() => setShowWebhookForm(false)} className="px-4 py-2 text-xs">Annuler</button></div>
                      </div>
                    )}
                    {webhooks.map(w => (<div key={w.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl"><div><p className="text-sm truncate">{w.url}</p></div><button onClick={() => handleDeleteWebhook(w.id)} className="text-red-400"><Trash2 size={14} /></button></div>))}
                  </div>
                </>
              )}
            </div>
