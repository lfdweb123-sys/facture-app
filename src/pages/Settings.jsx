  ...
  const handleRequestPushPermission = async () => {
    try {
      ...
      try {
        ...
        console.warn('Token non obtenu, activation basique:', tokenError.message);
      }

      // Toujours activer même sans token Firebase
      await updateDoc(doc(db, 'users', user.uid), {
        pushEnabled: true,
        updatedAt: new Date().toISOString()
      });
      ...
      toast.error('Erreur. Vérifiez que vous êtes sur HTTPS.');
    }
  };
        ...
        <div className="p-5 sm:p-6">
          ...
          {activeTab === 'profile' && (
            <div className="space-y-5">
              ...
              <button onClick={handleSaveProfile} disabled={loading} className="bg-gray-900 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-800 transition-all disabled:opacity-50 flex items-center gap-2">{loading ? <Loader className="animate-spin" size={14} /> : <Save size={14} />} Enregistrer</button>
            </div>
          )}
            ...
            <div className="space-y-5">
              ...
              {!isGoogle ? (
                <div className="space-y-4">
                  ...
                  <button onClick={handleChangePassword} disabled={loading || !passwordSettings.newPassword} className="bg-gray-900 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-800 transition-all disabled:opacity-50 flex items-center gap-2"><Key size={14} /> Mettre à jour</button>
                </div>
              ) : (
          ...
          {activeTab === 'notifications' && (
            <div className="space-y-5">
              ...
              <button onClick={handleSaveNotifications} disabled={loading} className="bg-gray-900 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-800 transition-all disabled:opacity-50 flex items-center gap-2">{loading ? <Loader className="animate-spin" size={14} /> : <Save size={14} />} Enregistrer</button>
            </div>
          )}

          {activeTab === 'invoices' && (
            <div className="space-y-5">
              <div><h3 className="text-sm font-semibold text-gray-900 mb-1">Paramètres de facturation</h3><p className="text-xs text-gray-500">Configuration par défaut</p></div>
              <div className="grid sm:grid-cols-2 gap-4">
                <div><label className={labelClass}>Délai paiement (j)</label><input type="number" value={invoiceSettings.delaiPaiement} onChange={e => setInvoiceSettings({...invoiceSettings, delaiPaiement: parseInt(e.target.value)||0})} className={inputClass} min="0" max="90" /></div>
                <div><label className={labelClass}>TVA par défaut (%)</label><input type="number" value={invoiceSettings.tauxTVADefaut} onChange={e => setInvoiceSettings({...invoiceSettings, tauxTVADefaut: parseInt(e.target.value)||0})} className={inputClass} min="0" max="100" /></div>
                ...
                <div><label className={labelClass}>Prochain N°</label><input type="number" value={invoiceSettings.prochainNumero} onChange={e => setInvoiceSettings({...invoiceSettings, prochainNumero: parseInt(e.target.value)||1})} className={inputClass} min="1" /></div>
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl"><div><p className="text-sm font-medium text-gray-900">Afficher la TVA</p></div><button onClick={() => setInvoiceSettings({...invoiceSettings, afficherTVA: !invoiceSettings.afficherTVA})} className={`relative w-11 h-6 rounded-full transition-colors ${invoiceSettings.afficherTVA?'bg-gray-900':'bg-gray-200'}`}><span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${invoiceSettings.afficherTVA?'translate-x-5':''}`} /></button></div>
              <button onClick={handleSaveInvoiceSettings} disabled={loading} className="bg-gray-900 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-800 transition-all disabled:opacity-50 flex items-center gap-2">{loading ? <Loader className="animate-spin" size={14} /> : <Save size={14} />} Enregistrer</button>
            </div>
          )}
          ...
          {activeTab === 'general' && (
            <div className="space-y-5">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div><label className={labelClass}>Format date</label><select className={inputClass} defaultValue="DD/MM/YYYY"><option value="DD/MM/YYYY">JJ/MM/AAAA</option></select></div>
                <div><label className={labelClass}>Fuseau horaire</label><select className={inputClass} defaultValue="Africa/Porto-Novo"><option value="Africa/Porto-Novo">Afrique/Porto-Novo</option></select></div>
              </div>
              <div className="p-4 bg-gray-50 rounded-xl text-sm"><div className="grid sm:grid-cols-2 gap-3"><div><p className="text-gray-500">ID utilisateur</p><p className="text-gray-900 font-mono text-xs truncate">{user?.uid}</p></div><div><p className="text-gray-500">Connexion</p><p className="text-gray-900">{isGoogle?'Google':'Email'}</p></div></div></div>
            </div>
          )}
