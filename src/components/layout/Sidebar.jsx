      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-30">
        <div className="flex items-center justify-around h-16">
          {menuItems.slice(0,5).map((item) => {
            ...
            const isDisabled = item.requireVerification && !isVerified;
            const Icon = item.icon;
            if (isDisabled) return <div key={item.path} className="flex flex-col items-center gap-0.5 px-3 py-2 text-xs text-gray-300 opacity-40"><Icon size={20}/><span className="truncate max-w-[60px]">{item.title}</span></div>;
            return <Link key={item.path} to={item.path} className={`flex flex-col items-center gap-0.5 px-3 py-2 text-xs font-medium ${isActive ? 'text-gray-900' : 'text-gray-400'}`}><Icon size={20}/><span className="truncate max-w-[60px]">{item.title}</span></Link>;
          })}
        </div>
      </nav>
    </>
